...
4. Defina as variáveis de ambiente `NODE_VERSION=20.18.1` e `NPM_CONFIG_PRODUCTION=false` (ambas já incluídas no `render.yaml`) — a segunda é essencial: o Render define `NODE_ENV=production` por padrão, o que faz o `npm install` pular `devDependencies` (onde ficam `vite`/`typescript`, necessários para o build) e quebra o build com `vite: not found`.
...
- **`npm error Exit handler never called!`**: bug conhecido do npm que costuma aparecer ao rodar múltiplos `npm install --prefix` encadeados em um único comando de build. Este projeto já evita esse padrão usando **npm workspaces** (uma única instalação na raiz) — mantenha `npm install --include=dev && npm run build` como está.
- **`sh: vite: not found` / `sh: tsc: not found`**: o host pulou `devDependencies` por causa de `NODE_ENV=production`. Garanta que o comando de instalação inclua `--include=dev` (ou defina `NPM_CONFIG_PRODUCTION=false`), como já está configurado no `render.yaml`.
