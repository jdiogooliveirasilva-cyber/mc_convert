...
/**
 * Expand any `.mcpack`/`.mcaddon` files found in the upload into their
 * contained pack folders, placed as virtual `behavior_packs/<name>/...` or
 * `resource_packs/<name>/...` siblings right next to wherever the archive
 * file itself was found (inside the world, or alongside it). This lets
 * users drop these files directly — exactly as exported by Minecraft or
 * downloaded from a marketplace — instead of having to manually unzip them
 * into a `behavior_packs`/`resource_packs` folder first.
 *
 * Both formats are themselves ZIP archives:
 * - `.mcpack`: a single pack (one `manifest.json`, usually at its root).
 * - `.mcaddon`: one or more packs bundled together (each in its own
 *   subfolder, each with its own `manifest.json`).
 *
 * Pack type (behavior vs. resource) is read from the manifest's `modules`
 * array — never guessed from the file name — and the manifest bytes
 * themselves are never parsed-and-rewritten, only relocated, so UUIDs and
 * versions are preserved exactly as authored.
 */
async function expandPackArchiveEntries(
  entries: ZipEntryRef[],
): Promise<{ expanded: ZipEntryRef[]; importedCount: number }> {
  const expanded: ZipEntryRef[] = [];
  let importedCount = 0;
  ...
  for (const ref of entries) {
    ...
    const parentPrefix = parentPrefixOf(ref.path);
    const baseName = ref.path
      .slice(parentPrefix.length)
      .replace(/\.(mcpack|mcaddon)$/i, "");

    try {
      const buf = await ref.entry.async("nodebuffer");
      const innerZip = await JSZip.loadAsync(buf);
      const innerEntries: ZipEntryRef[] = [];
      innerZip.forEach((relPath, entry) => {
        if (entry.dir) return;
        innerEntries.push({ path: relPath.replace(/\\/g, "/"), entry });
      });

      const manifestDirs = new Set<string>();
      for (const { path } of innerEntries) {
        if (path === "manifest.json" || path.endsWith("/manifest.json")) {
          manifestDirs.add(path === "manifest.json" ? "" : path.slice(0, -"manifest.json".length));
        }
      }

      if (manifestDirs.size === 0) {
        // Not a recognizable pack archive — keep the original file untouched
      ...
      for (const dir of manifestDirs) {
        subIndex++;
        const manifestRef = innerEntries.find((e) => e.path === `${dir}manifest.json`);
        let kind: "behavior" | "resource" = "behavior";
        if (manifestRef) {
          try {
            const raw = await manifestRef.entry.async("text");
            const manifest = JSON.parse(raw) as { modules?: Array<{ type?: unknown }> };
            if (
              Array.isArray(manifest.modules) &&
              manifest.modules.some((m) => m && m.type === "resources")
            ) {
              kind = "resource";
            }
          } catch {
            // Malformed manifest — keep the "behavior" default guess; the
            // files are still imported and the missing/invalid manifest is
            // reported the same way as any other pack folder.
          }
        }

        const folderName =
          manifestDirs.size > 1
            ? sanitizeWorldFileName(`${baseName}_${dir.replace(/\/$/, "") || subIndex}`)
            : sanitizeWorldFileName(baseName);
        const topDir = kind === "behavior" ? "behavior_packs" : "resource_packs";

        for (const { path, entry } of innerEntries) {
          if (!path.startsWith(dir)) continue;
          const rest = path.slice(dir.length);
          if (rest.length === 0) continue;
          expanded.push({ path: `${parentPrefix}${topDir}/${folderName}/${rest}`, entry });
        }
      }

      importedCount++;
    } catch {
      // Corrupted/unreadable .mcpack or .mcaddon — keep the raw file so
      // nothing is silently lost, even though it won't be recognized as a
      // pack.
      expanded.push(ref);
    }
  }

  return { expanded, importedCount };
}
...
export async function convertWorldToMcworld(zipBuffer: Buffer): Promise<ConvertResult> {
  ...
  const rawKeptEntries = allEntries.filter((e) => !isJunkPath(e.path));

  const { expanded: keptEntries, importedCount: mcpackImportedCount } =
    await expandPackArchiveEntries(rawKeptEntries);
  ...
  if (mcpackImportedCount > 0) {
    fixesApplied.push(
      `${mcpackImportedCount} arquivo(s) .mcpack/.mcaddon foram detectados e importados automaticamente como addons do mundo.`,
    );
  }
